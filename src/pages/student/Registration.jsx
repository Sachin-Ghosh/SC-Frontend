...
const Registration = () => {
  ...
  useEffect(() => {
    ...
    const getTeamMembers = async () => {
      try {
        const response = await fetch(`${import.meta.env.VITE_API_URL}/api/users/filter/?department=${encodeURIComponent(formData.department)}&year=${formData.year ? formData.year:''}&division=${formData.division ? formData.division:''}&page=1&page_size=10000`, {
          method: 'GET',
          headers: {
            'Authorization': `Bearer ${accessToken}`,
          }
